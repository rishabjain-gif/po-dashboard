    ...
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-4 items-center">
        ...
        <div className="ml-auto flex items-center gap-4 text-xs text-slate-500">
          <span className="flex items-center gap-1.5"><FillDot rate={100} /> 100%</span>
          <span className="flex items-center gap-1.5"><FillDot rate={85} /> 80–99%</span>
          <span className="flex items-center gap-1.5"><FillDot rate={50} /> &lt;80%</span>
        </div>
      </div>
            ...
            <tr className="bg-slate-50 border-b border-slate-200 text-xs text-slate-400">
              ...
              {data.weeks.map((_, i) => (
                <React.Fragment key={i}>
                  <th className="px-3 py-2 text-center border-l border-slate-200 font-medium">Ordered</th>
                  <th className="px-3 py-2 text-center font-medium">Shipped</th>
                  <th className="px-3 py-2 text-center font-medium">Fill %</th>
                </React.Fragment>
              ))}
            </tr>
          </thead>
                ...
                {row.weeks.map((w, i) => (
                  <React.Fragment key={i}>
                    <td className="px-3 py-3 text-center border-l border-slate-100 text-slate-700 tabular-nums">
                      {w.ordered > 0 ? w.ordered.toLocaleString() : <span className="text-slate-300">—</span>}
                    </td>
                    <td className="px-3 py-3 text-center text-slate-700 tabular-nums">
                      {w.shipped > 0 ? w.shipped.toLocaleString() : <span className="text-slate-300">—</span>}
                    </td>
                    <td className="px-3 py-3 text-center">
                      {w.fillRate !== null ? <FillBadge rate={w.fillRate} /> : <span className="text-slate-300">—</span>}
                    </td>
                  </React.Fragment>
                ))}
              </tr>
      ...
      <p className="text-xs text-slate-400 mt-3">
        {filtered.length} of {data.data.length} SKU-platform combinations
      </p>
    </div>
  );
}
